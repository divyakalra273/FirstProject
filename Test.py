...
class Fee:
    ...
    def inputFeeDetails(self):
        ...
        if os.path.isfile("gc-fees.csv"):
            ...
            if difference == 1:
                ...
                self.amount = (((feeStructure[size]) * (difference - 1)) + (
                            (feeStructure[size] * 0.10) * (difference - 1))) + self.amount
                print(">> Fee Not Paid for {} months.Now the Total Amount to be paid is \u20b9 {} ".format(difference, self.amount))
                self.saveFees()
    ...
    def saveFees(self):
        ...
        save = input("Would you like to save Fee {} for {} (yes/no)".format(self.amount, self.phone))
        if save == "yes":
